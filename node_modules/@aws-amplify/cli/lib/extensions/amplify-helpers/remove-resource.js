...
async function forceRemoveResource(context, category, name, dir) {
    ...
    if (!amplifyMeta[category] || Object.keys(amplifyMeta[category]).length === 0) {
        ...
        context.usageData.emitError(new amplify_cli_core_1.ResourceDoesNotExistError());
    ...
    if (!context || !category || !name || !dir) {
        ...
        context.usageData.emitError(new amplify_cli_core_1.MissingParametersError());
        amplify_cli_core_1.exitOnNextTick(1);
    }
...
async function removeResource(context, category, resourceName, questionOptions = {}) {
    ...
    let enabledCategoryResources = Object.keys(amplifyMeta[category]).filter(r => !!amplifyMeta[category][r].providerPlugin);
    if (resourceName) {
