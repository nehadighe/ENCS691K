...
function moveBackendResourcesToCurrentCloudBackend(resources) {
    ...
    for (let i = 0; i < resources.length; i += 1) {
        const sourceDir = path.normalize(path.join(amplify_cli_core_1.pathManager.getBackendDirPath(), resources[i].category, resources[i].resourceName));
        const targetDir = path.normalize(path.join(amplify_cli_core_1.pathManager.getCurrentCloudBackendDirPath(), resources[i].category, resources[i].resourceName));
...
function updateamplifyMetaAfterResourceAdd(category, resourceName, options = {}) {
    ...
    update_backend_config_1.updateBackendConfigAfterResourceAdd(category, resourceName, options);
}
