...
exports.addTrigger = async (triggerOptions) => {
...
exports.updateTrigger = async (triggerOptions) => {
    const { key, values, context, functionName, triggerEnvs = '[]', category, parentStack, targetPath, parentResource, triggerIndexPath, triggerPackagePath, triggerDir, triggerTemplate, triggerEventPath, skipEdit, } = triggerOptions;
    let update;
    try {
    ...
    try {
        ...
        if (values && values.length > 0) {
            ...
            if (dirContents.includes('parameters.json')) {
                amplify_cli_core_1.JSONUtilities.writeJson(path.join(parametersPath, 'parameters.json'), { modules: values.join() });
            }
...
exports.deleteDeselectedTriggers = async (currentTriggers, previousTriggers, functionName, targetDir, context) => {
...
exports.deleteTrigger = async (context, name, dir) => {
    try {
...
exports.deleteAllTriggers = async (triggers, functionName, dir, context) => {
...
exports.triggerFlow = async (context, resource, category, previousTriggers = {}) => {
    if (!resource)
...
exports.getTriggerPermissions = async (context, triggers, category) => {
    let permissions = [];
...
exports.choicesFromMetadata = (triggerPath, selection, isDir) => {
...
exports.getTriggerMetadata = (triggerPath, selection) => amplify_cli_core_1.JSONUtilities.readJson(`${triggerPath}/${selection}.map.json`);
...
exports.copyFunctions = async (key, value, category, context, targetPath) => {
    try {
...
exports.cleanFunctions = async (key, values, category, context, targetPath) => {
    const pluginPath = context.amplify.getCategoryPluginInfo(context, category).packageLocation;
    try {
...
exports.getTriggerEnvVariables = (context, trigger, category) => {
    const pluginPath = context.amplify.getCategoryPluginInfo(context, category).packageLocation;
    let env = [];
...
exports.getTriggerEnvInputs = async (context, triggerPath, triggerKey, triggerValues, currentEnvVars) => {
...
exports.dependsOnBlock = (context, triggerKeys = [], provider) => {
    if (!context)
